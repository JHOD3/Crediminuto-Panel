...
(function () {
  ...
  function escapeHtml(html) {
    return html.replace(/×/g, '&times;').replace(/«/g, '&laquo;').replace(/»/g, '&raquo;').replace(/←/g, '&larr;').replace(/→/g, '&rarr;');
  }
  ...
  addSourceModals(); // Prevent empty `a` elements or `submit` buttons from navigating away
  ...
  for (const element of bsComponents) {
    const button = '<button class="source-button btn btn-primary btn-xs py-2" type="button" tabindex="0">{>}</button>';
...
/******/ })()
;
//# sourceMappingURL=uikit.00170efabac3a0e8f00f.js.map